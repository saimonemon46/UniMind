...
const navItems: Record<Role, NavItem[]> = {
  admin: [
    { label: 'Dashboard', to: '/admin', icon: LayoutDashboard }, { label: 'Users', to: '/admin/users', icon: Users }, { label: 'Departments', to: '/admin/departments', icon: BookOpen }, { label: 'Courses & enrollment', to: '/admin/courses', icon: GraduationCap }, { label: 'Schedules', to: '/admin/schedules', icon: CalendarDays }, { label: 'Analytics', to: '/admin/analytics', icon: BarChart3 },
  ],
  faculty: [
...
export function Sidebar({ role }: SidebarProps) {
  return <aside className="hidden min-h-screen w-64 border-r border-ink-10 bg-white px-4 py-5 lg:block"><div className="px-3"><p className="font-serif text-2xl font-normal tracking-tight text-ink">Alma</p><p className="mt-1 text-xs text-ink-60">University Management</p></div><nav className="mt-8 space-y-1">{navItems[role].map(({ label, to, icon: Icon }) => <NavLink key={to} to={to} end className={({ isActive }) => `flex items-center gap-3 rounded-sm px-3 py-2 text-sm text-ink-60 ${isActive ? ROLE_ACCENTS[role].active : 'hover:bg-sand-100'}`}><Icon className="h-4 w-4" aria-hidden="true" /><span>{label}</span></NavLink>)}</nav></aside>
}
